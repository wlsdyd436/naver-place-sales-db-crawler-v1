...
- 2026-08-04 실제 Python GUI 시나리오 검증 완료: 기본 수집·홈페이지·SNS 수집·법정동 필터·신규오픈 전용·리뷰 필터·일시정지·재개·사용자 중지·부분 저장·Excel 14열 계약을 실 사용자 실행으로 확인. CAPTCHA·429·navigation error는 의도적으로 유발하지 않았으며 이번 검증 중 자연 발생하지도 않았다. EXE 검증은 아직 미완료다.

이 검증은 자동화된 단위·계약 테스트와 실 GUI 시나리오 검증 기준이다. EXE 최종 실행과 실제 네이버 환경 전반에서의 장기적 동작은 이 결과가 보장하지 않는다.
