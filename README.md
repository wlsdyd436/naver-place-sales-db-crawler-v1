...
## 9. place_id 기반 상세정보 enrichment(2026-07-21 추가, 기본 흐름에는 미연결)

`DomMembershipCollector.enrich_detail(rows, max_targets=None)`이 제공합니다. **현재 UI 기본 "수집 시작" 흐름에서 자동으로 호출되지 않습니다** - 300건 전체를 상세 페이지까지 순회하면 소요 시간과 네이버 측 차단 위험이 늘어나므로, 이 기능은 코드에서 명시적으로 호출해야 하는 별도 capability로 남겨두었습니다.

- **플레이스 URL 생성**: place_id로 `https://pcmap.place.naver.com/place/{id}/home` 형태(업종별 URL 세그먼트를 추측하지 않는 범용 형태)를 만듭니다. 2026-07-21 Live 검증에서 300/300 생성 확인 + 10건 실제 방문에서 전부 정상 렌더링·업체명 일치가 확인되어, 임의의 vertical(restaurant/hospital 등)을 하드코딩하지 않고도 실제로 동작함이 실측으로 검증되었습니다.
- **상세 페이지 방문**: place_id로 만든 URL을 직접 열어 대표전화/주소/홈페이지/인스타/블로그를 확보합니다. 추출 로직은 기존 `src/pc/detail_scraper.py`(카드 클릭 기반 legacy 상세 수집기)의 검증된 함수를 그대로 재사용합니다 - 항목당 새로 만든 부분은 "place_id로 직접 URL 진입"뿐입니다.
...
- **일부 업체는 전화·외부 링크가 없을 수 있습니다.** 상세 페이지 자체에 해당 정보가 없으면 빈 칸으로 남으며, 이는 실패가 아닙니다(2026-07-21 10건 표본에서 대표전화 10/10, 인스타 3/10, 홈페이지 2/10, 블로그 0/10 확인 - 업체마다 실제로 등록한 채널이 다릅니다).
- **300건 전체 상세 방문은 아직 실측되지 않았습니다.** 10건 표본 기준 평균 약 1.24초/건이었으나, 이를 300건으로 단순 외삽한 값일 뿐 실제 대량 방문 시의 차단 위험은 검증되지 않았습니다.
