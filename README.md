...
- **법정동/역상권 세부구역은 현재 강동구 샘플 데이터로만 실험적으로 연결되어 있고, 전 지역 대상 법정동 데이터 연결·지역 exact 필터는 아직 다음 단계 작업입니다.** 다른 구를 선택하면 구 단위 fallback 검색으로 동작합니다(PROJECT_STATE.md 남은 작업 참고).
