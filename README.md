...
## 9. place_id 기반 상세정보 enrichment(선택적 capability - 기본 흐름에는 미연결)

**기본 Network/List "수집 시작" 흐름은 DOM + React Fiber(§8)로 place_id를 확보하고, 목록 Network/Apollo 응답으로 필드를 보강한 뒤 곧바로 Excel로 저장합니다. 업체별 상세 페이지 자동 방문은 없습니다.** 대표전화/홈페이지/인스타/블로그처럼 목록 데이터 자체에 없는 필드는 공란으로 남을 수 있습니다.

`DomMembershipCollector.enrich_detail(rows, *, max_targets=None, should_continue=None, on_progress=None)`이 상세정보 보강 기능을 제공하지만, **현재 기본 UI "수집 시작" 경로에서 자동으로 호출되지 않습니다** - 300건 전체를 상세 페이지까지 순회하면 소요 시간과 네이버 측 차단 위험이 늘어나므로, 코드에서 명시적으로 호출해야 하는 별도 capability로 남겨두었습니다.

- **플레이스 URL 생성**: place_id로 `https://pcmap.place.naver.com/place/{id}/home` 형태(업종별 URL 세그먼트를 추측하지 않는 범용 형태)를 만듭니다.
- **상세 페이지 방문**: place_id로 만든 URL을 직접 열어 대표전화/주소/홈페이지/인스타/블로그를 확보합니다. 추출 로직은 기존 `src/pc/detail_scraper.py`(카드 클릭 기반 legacy 상세 수집기)의 검증된 함수를 그대로 재사용합니다.
- **진행률/사용자 중지 콜백**: `should_continue`(매 row 및 재시도 직전 확인, False가 되면 새 방문 없이 남은 행을 원본 그대로 보존)와 `on_progress(completed, total, success_count, failure_count)`(매 row 처리 후 호출, 콜백 예외는 무시)를 지원하지만, 둘 다 기본 UI 경로에는 연결되어 있지 않습니다.
- **안전 정책**: 순차 실행(동시성 없음), 항목당 최대 1회 재시도, 업체명이 상세 페이지 제목과 일치하지 않으면 병합하지 않음(오염 방지), CAPTCHA·HTTP 403·405·429 감지 시 즉시 중단, 연속 실패가 임계(5건)를 넘어도 중단합니다. 어느 경우든 이미 처리된 결과와 아직 처리하지 않은 원본 행은 그대로 보존됩니다(행 삭제 없음).
- **선택형 상세 엔진 검증 기록(2026-07-21, 별도 호출로 실측)**: 10건 표본에서 플레이스 URL 300/300 생성 + 10건 실제 방문 전부 정상 렌더링·업체명 일치, 대표전화 10/10·인스타 3/10·홈페이지 2/10·블로그 0/10 확인. 이어 30건 표본에서 대표전화 27/30·인스타 13/30·홈페이지 9/30·블로그 1/30 확인, 평균 약 0.9초/건(27.09초/30건), 오매칭·CAPTCHA·HTTP 403/405/429 없음 - 모두 `enrich_detail`을 별도로 호출해 얻은 결과이며, 기본 "수집 시작" 흐름에서 발생한 것이 아닙니다.
- **300건 전체 상세 방문은 실측되지 않았습니다.** 위 30건 표본 기준 단순 외삽 시 약 4~5분이 추가로 소요될 것으로 예상되나, 대량 연속 방문 시의 차단 위험은 검증되지 않았습니다.
