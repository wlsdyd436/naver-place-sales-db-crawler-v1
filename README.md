...
- **지역 선택은 행정안전부 공식 법정동 Snapshot(§10/§11) 하나만 사용합니다.** 앱 실행 중 행안부 API를 호출하지 않으며(로컬 파일만 읽음), 전국 시도/시군구/법정동을 전부 선택할 수 있습니다. 역·상권/세부업종은 여전히 강동구 샘플 데이터로만 채워지는 보조 검색 힌트이며(§11), 지역 exact 필터와 네이버 검색어 명칭 호환성 검증은 아직 다음 단계 작업입니다(PROJECT_STATE.md 남은 작업 참고).
...
- **시도/시군구는 콤보박스 단일 선택**입니다. 시도를 바꾸면 시군구·법정동 선택이 초기화되고, 시군구를 바꾸면 법정동 선택이 초기화됩니다. 세종특별자치시처럼 시군구 계층이 없는 지역은 시군구 콤보박스가 비활성화되고 법정동을 바로 선택할 수 있습니다(지역명을 코드에 하드코딩하지 않고 데이터 구조로만 판단).
- **법정동은 "법정동 선택" 버튼으로 여는 팝업에서 다중 선택**합니다(전체 선택/해제 제공). 법정동을 하나도 선택하지 않으면 시군구(시군구 없는 지역은 시도) 단위로 검색 조합 1개를 만들고, 여러 개 선택하면 법정동마다 검색 조합을 만듭니다.
- **"2. 선택 사항"의 역·상권/세부업종은 별도의 보조 검색 힌트**이며, 현재 선택된 시도/시군구를 기준으로 `data/regions_kr_sample.json`(강동구 샘플만 존재)을 조회합니다. 법정동은 이 샘플 데이터에서 읽지 않고 공식 Snapshot에서만 읽습니다. 데이터가 없는 지역(대부분)은 오류가 아니라 "준비 중" 안내만 표시되며, 공식 법정동 검색 자체는 정상 동작합니다. 선택된 보조 항목은 공식 법정동 검색 조합에 추가로 합쳐지고 중복은 제거됩니다.
- **전체 목표 저장 개수는 항상 자동 계산되며 직접 입력할 수 없습니다**(검색 조합당 수집 상한 × 최종 검색 조합 수 - 공식 법정동 + 보조 선택을 합쳐 중복 제거한 결과 기준). 입력창은 항상 읽기 전용(disabled)입니다.
- 두 수집 모드("빠른 기본 수집"/"홈페이지·SNS 포함 수집") 모두 동일한 검색 조합과 자동 계산된 목표 개수를 사용합니다.
- 세종특별자치시·전남광주통합특별시 등은 행정안전부 공식 명칭을 그대로 표시/사용합니다. **네이버 검색어와의 호환성은 아직 검증되지 않았으며, 지역 exact 필터도 다음 단계 작업입니다.**
