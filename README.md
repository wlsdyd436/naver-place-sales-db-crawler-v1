...
## 9. place_id 기반 상세정보 enrichment(2026-07-21부터 목록 수집 후 자동 실행)

`DomMembershipCollector.enrich_detail(...)`이 제공하며, **목록 300건 수집이 끝나면 같은 실행 안에서 자동으로 이어집니다**(`app.py`의 "수집 시작" 버튼 하나로 목록 → 상세정보 → Excel 저장까지 자동 진행, 별도 화면 옵션 없음). 예전 엔진(`NetworkBrowserCollector`)으로 명시적으로 되돌린 경우에는(§8) 이 단계 자체가 없습니다(`enrich_detail`이 없는 collector를 감지하면 건너뜁니다).

- **플레이스 URL 생성**: place_id로 `https://pcmap.place.naver.com/place/{id}/home` 형태(업종별 URL 세그먼트를 추측하지 않는 범용 형태)를 만듭니다. 300/300 생성 + 실제 방문(10건, 30건 두 차례) 전부 정상 렌더링·업체명 일치가 확인되어, vertical을 하드코딩하지 않고도 동작함이 실측 검증되었습니다.
...
- **진행 상태**: 목록 수집 완료 → "상세정보 수집 준비 중" → "상세정보 수집 중 N/300 (성공 X / 실패 Y)"가 기존 진행률 표시줄(progress bar)과 상태 문구에 실시간으로 반영됩니다 → "상세정보 수집 완료" → "Excel 저장 중" 순으로 표시됩니다.
- **사용자 중지**: 기존 "중지" 버튼(`stop_event`)을 상세 단계에서도 그대로 재사용합니다. 중지하면 새로운 상세 방문을 즉시 멈추고, 이미 확보한 상세정보는 유지하며, 아직 방문하지 않은 업체는 목록 데이터 상태 그대로 남습니다. 300개 행 자체는 항상 보존되고, 그 시점까지의 결과가 Excel로 저장됩니다.
...
- **일부 업체는 전화·외부 링크가 없을 수 있습니다.** 상세 페이지 자체에 해당 정보가 없으면 빈 칸으로 남으며, 이는 실패가 아닙니다(2026-07-21 30건 표본: 대표전화 27/30, 인스타 13/30, 홈페이지 9/30, 블로그 1/30 확인 - 업체마다 실제로 등록한 채널이 다릅니다).
- **예상 소요시간**: 30건 표본 기준 평균 약 0.9초/건, 300건 전체로 단순 외삽하면 약 4~5분이 목록 수집 시간에 더해집니다(실측이 아닌 외삽치이며, 대량 연속 방문 시의 차단 위험은 300건 전체로는 아직 검증되지 않았습니다).
